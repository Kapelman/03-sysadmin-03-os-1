...
```
- Находим по Pstree -p PID процесса - 37973 , видим метку deleted

```
vagrant@vagrant:~$ sudo lsof -p 37973  
ping      37973                     1000    1w      REG              253,0   186982    1055328 /home/vagrant/file_ping (deleted)
ping      37973                     1000    2w      REG              253,0   186982    1055328 /home/vagrant/file_ping (deleted)
```

- Освобожение места возможно двумя способами.
Способ 1. Использовать команду truncate, место занятое 
удаленным файлом освободится (этот способ  не помог, удаленный файл продолжил расти, хотя truncate отработал без сообщений).
```

vagrant@vagrant:~$ sudo truncate -s 0 /proc/37973/fd/1
vagrant@vagrant:~$ echo $?
0
vagrant@vagrant:~$ sudo truncate -s 0 /proc/37973/fd/2

ping    37973 vagrant    1w   REG  253,0   208102 1055328 /home/vagrant/file_ping (deleted)
ping    37973 vagrant    2w   REG  253,0   208102 1055328 /home/vagrant/file_ping (deleted)
```
...
Например:
```
vagrant@vagrant:~$ sudo echo >/proc/37973/fd/1
-bash: /proc/37973/fd/1: Permission denied

vagrant@vagrant:~$ sudo echo >/proc/37973/fd/2
-bash: /proc/37973/fd/2: Permission denied
```
